					    ...
					    <tr data-id="row_{{ $savingsproduct->id }}">
							...
							<td class="text-center">
								<span class="dropdown">
								  ...
								  <form action="{{ action('SavingsProductController@destroy', $savingsproduct['id']) }}" method="post">
									{{ csrf_field() }}
									<input name="_method" type="hidden" value="DELETE">

									<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
										<a href="{{ action('SavingsProductController@edit', $savingsproduct['id']) }}" data-title="{{ _lang('Update Account Type') }}" class="dropdown-item dropdown-edit ajax-modal"><i class="ti-pencil-alt"></i>&nbsp;{{ _lang('Edit') }}</a>
										<a href="{{ action('SavingsProductController@show', $savingsproduct['id']) }}" data-title="{{ _lang('Account Type Details') }}" class="dropdown-item dropdown-view ajax-modal"><i class="ti-eye"></i>&nbsp;{{ _lang('View') }}</a>
